...
const MembersForum: FC<Props> = ({ moderator }: Props): JSX.Element => {
  const { id } = useParams()
  const { currentUserSuccess } = useStoreState(userStateSelector)
  const { updateStatusUserFromForum, addUserToForum, setListUserForum } =
    useStoreActions(forumActionSelector)
  const { listUserForum } = useStoreState(forumStateSelector)
  const { setNotifySetting } = useStoreActions(notifyActionSelector)

  const [listUser, setListUser] = useState<IUserData[]>(listUserForum)
  ...
  const handleAddUser = async (data: any): Promise<void> => {
    ...
    if (id) {
      ...
      if (res) {
        ...
        setListUserForum([...listUser, ...data])
      }
    }
  }
      ...
      <div className="p-4 my-4 bg-white min-h-[100px] shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] rounded-md ">
        <div className="flex items-center justify-between">
          <h4 className="font-medium text-base">
            Members{' '}
            <span className="ml-2 font-thin text-sm"> {listUserForum?.length}</span>
          </h4>
