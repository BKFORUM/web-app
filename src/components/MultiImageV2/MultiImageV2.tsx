...
const MultiImage = ({
  ...
  return (
    <div>
      <div
        className={`cursor-pointer group mb-2 grid grid-cols-2 gap-2`}
        {...getRootProps({
          onClick: (e: any) => handleClick(e),
        })}>
        <div className={`rounded border-gray-300 gap-4 h-[200px] ${hiddenBtnAdd}`}>
          <div
            className={`relative w-full h-full object-cover aspect-[3/2] rounded-md border-2 border-dashed border-amber-400 bg-[#fef9ee] flex justify-center items-center select-none`}>
            <button
              style={{ opacity: 0.8 }}
            ...
            <label
              ...
              Thêm ảnh ({listImage.length}/{maxItem})
            </label>
          </div>
        </div>
