...
const WritingTools: FC<Props> = ({
  ...
  return (
    <div
      className={`flex gap-2 relative ${type === 'comment' ? 'px-3 py-2' : 'py-1'}`}
      style={{
        boxShadow: type === 'comment' ? '0px 0px 10px 0px rgba(0, 0, 0, 0.1)' : '',
      }}>
      <Tooltip title={currentUserSuccess?.fullName}>
        <div
          className={`overflow-hidden ${type === 'comment' ? 'h-10 w-10' : 'h-7 w-7'} `}>
          <img
      ...
      <div className="flex-1 m-auto relative">
        <textarea
          ...
          className={`w-full bg-gray-200 outline-none rounded-3xl text-sm z-10 ${
            type === 'comment' ? ' px-4 pt-2' : 'px-4 h-7 pt-1'
          } `}
